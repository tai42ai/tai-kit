...
This module is behind the ``tai-kit[jwt]`` optional extra: it imports ``joserfc``
at module top (the kit driver-extra pattern), so it is deliberately NOT
re-exported from :mod:`tai_kit.net` — a plain ``import tai_kit.net`` must keep
working for every install that did not opt into ``[jwt]``. Consumers import from
here directly: ``from tai_kit.net.jwt import verify_jwt``.

Issuer discovery and JWKS URLs are OPERATOR CONFIG, not caller-supplied input, so
these fetches do NOT run through the SSRF guard that :mod:`tai_kit.net.fetch_url`
