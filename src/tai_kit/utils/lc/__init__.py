...
(and ``tai_kit.utils.lc.signature_util``) imports without the LangChain stack,
and touching an lc_util name without it raises a ``ModuleNotFoundError`` naming
the ``tai-kit[llm]`` extra.
"""
...
def __getattr__(name: str) -> object:
    ...
    try:
        lc_util = import_module("tai_kit.utils.lc.lc_util")
        ...
        if exc.name is not None and exc.name.partition(".")[0].startswith("langchain"):
            raise ModuleNotFoundError(
                ...
                f"(tai-kit[llm]) to use it. Missing module: {exc.name!r}.",
                name=exc.name,
            ) from exc
        raise
